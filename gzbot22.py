...
def processTime(t):
    # 确保字符串是有效的 ISO 格式，包含时区信息
    t_truncated = t[:23]  # 截断到秒
    input_time = datetime.fromisoformat(t_truncated)  # 解析 ISO 格式字符串
    input_time_utc = input_time.replace(tzinfo=timezone.utc)  # 设置为 UTC 时区
    beijing_timezone = timezone(timedelta(hours=8))  # 北京时区
    beijing_time = input_time_utc.astimezone(beijing_timezone)  # 转换为北京时间
    return beijing_time.strftime("%Y-%m-%d %H:%M:%S")  # 返回格式化的时间字符串
    ...
    try:
        while True:
            ...
            if (firstbloodLen < tmpFirstBloodInfoLen):
                team_name = tmpFirstBloodInfo[0]['content'][0]
                challenge_name = tmpFirstBloodInfo[0]['content'][1]
                message = "%s\n===【一血】===\n恭喜队伍：%s \n解出题目：%s \n真的是太强了！" % (processTime(tmpFirstBloodInfo[0]['time']), team_name, challenge_name)
            ...
            if (secondbloodLen < tmpSecondBloodInfoLen):
                team_name = tmpSecondBloodInfo[0]['content'][0]
                challenge_name = tmpSecondBloodInfo[0]['content'][1]
                message = "%s\n===【二血】===\n恭喜队伍：%s \n解出题目：%s \n666简直是太棒了" % (processTime(tmpSecondBloodInfo[0]['time']), team_name, challenge_name)
            ...
            if (thirdbloodLen < tmpThirdBloodInfoLen):
                team_name = tmpThirdBloodInfo[0]['content'][0]
                challenge_name = tmpThirdBloodInfo[0]['content'][1]
                message = "%s\n===【三血】===\n恭喜队伍：%s \n解出题目：%s \n Orz" % (processTime(tmpThirdBloodInfo[0]['time']), team_name, challenge_name)
